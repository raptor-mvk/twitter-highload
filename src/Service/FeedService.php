...
final class FeedService
{
    ...
    public function getFeed(int $userId, int $count): array {
        ...
        return $feed === null ? [] : array_slice($feed->tweets, -$count);
    }

    public function putTweet(Tweet $tweet, int $readerId): bool {
        $feed = $this->getFeedFromRepository($readerId);
        if ($feed === null) {
            return false;
        }
        $feed->tweets[] = $tweet->toFeed();
